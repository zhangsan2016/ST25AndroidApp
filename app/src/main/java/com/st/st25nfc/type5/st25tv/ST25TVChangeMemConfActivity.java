...
public class ST25TVChangeMemConfActivity extends STFragmentActivity implements STType5PwdDialogFragment.STType5PwdDialogListener, NavigationView.OnNavigationItemSelectedListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        View childView = getLayoutInflater().inflate(R.layout.fragment_st25tv_change_mem_conf, null);
        ...
        mSingleAreaRadioButton = (RadioButton) findViewById(R.id.singleAreaRadioButton);
        mTwoAreasRadioButton = (RadioButton) findViewById(R.id.twoAreasRadioButton);
    ...
    private void getCurrentMemoryConf() {
        new Thread(new Runnable() {
            public void run() {
                final boolean isMemoryConfiguredInSingleArea;
                try {
                    isMemoryConfiguredInSingleArea = (myTag.getNumberOfAreas() > 1) ? false : true;
    ...
    private void changeTagMemConf() {
        new Thread(new Runnable() {
            public void run() {
                Log.v(TAG, "changeTagMemConf");

                RadioButton singleAreaRadioButton = (RadioButton) findViewById(R.id.singleAreaRadioButton);

                try {
                    if (singleAreaRadioButton.isChecked()) {
                        myTag.setNumberOfAreas(1);
                    } else {
                        myTag.setNumberOfAreas(2);
                    }
                    showToast(R.string.tag_updated);

                    // TODO: Reparse all the tag because memory configuration has changed
