...
public class Type5LockBlockActivity extends STFragmentActivity implements STType5PwdDialogFragment.STType5PwdDialogListener, NavigationView.OnNavigationItemSelectedListener {
    ...
    private void checkBlockStatus() {
        new Thread(new Runnable() {
            public void run() {
                ...
                int blockNbr;
                try {
                    blockNbr = Integer.parseInt(blockNbrEditText.getText().toString());
                } catch (Exception e) {
                    STLog.e("Bad block number" + e.getMessage());
                    showToast(R.string.bad_block_number);
                    return;
                }
                ...
                try {
                    if(blockNbr < 256) {
                        readBlockAnswer = myTag.readSingleBlock(blockNbr, flag);
                    } else {
                        readBlockAnswer = myTag.extendedReadSingleBlock(blockNbr, flag);
                    }
                } catch (STException e) {
                    switch (e.getError()) {
    ...
    private void askConfirmationBeforeLocking() {
        ...
        // set dialog message
        alertDialogBuilder
                .setMessage(getString(R.string.warning_irreversible_action_lock_block_confirmation_needed))
                .setCancelable(true)
        ...
        new Thread(new Runnable() {
            public void run() {
                ...
                try {
                    ...
                    if(blockNbr < 256) {
                        result = myTag.lockSingleBlock(blockNbr);
                    } else {
                        result = myTag.extendedLockSingleBlock(blockNbr);
                    }
